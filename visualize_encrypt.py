...
import csv
from cipher_core import signal_spiral_encrypt, derive_key_from_ec  # Import EC key derivation
...
def visualize_encryption(block, key, rounds=16, save=False, filename="encryption_visual.png",
    ...
    Note: signal_spiral_encrypt implements 256-bit encryption with non-integer Collatz dynamics.
    """
...
def main():
    ...
    parser.add_argument("--use-ec", action="store_true", help="Use elliptic curve derived key")
    ...
    # Convert hex to float if needed
    block = float.fromhex(block) if block.lower().startswith("0x") else float(block)
    key = float.fromhex(key) if isinstance(key, str) and key.lower().startswith("0x") else float(key)
